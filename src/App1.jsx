...
function App() {
  ...
  // Stations already tested during the current skip/recovery cycle
  const failedStationsRef = useRef(new Set());

  // Configurable station health system
  ...
  const displayedRegionStations = selectedRegion === "ALL" ? regionStations : regionStations.filter(
    (station) => getStationRegion(station) === selectedRegion);
  // const displayedStations = stationFilter === "favorites" ? favorites : stations;
  const displayedStations = stationFilter === "favorites" ? favorites : stationFilter === "regions" ? displayedRegionStations : stations;
  const activeStationList = stationFilter === "region" ? regionStations : stationFilter === "favorites" ? favorites : stations;
  // activeStationListRef.current = activeStationList;
...
//   totalRegionStations: regionStations.length,
//   availableRegions,
//   selectedRegion,
//   displayedRegionStations: displayedRegionStations.length,
//   stations: displayedRegionStations.map((station) => ({
//     name: station.name,
...
function isFavorite(station) {
  ...
  try {
    ...
    // Keep only stations that have a region/state
...
async function loadRegionStations() {
  ...
  const activeStations = getActiveStationList();

  console.log("SEEK ACTIVE LIST:", {
  filter: stationFilter,
  selectedRegion,
  count: activeStations.length,
  stations: activeStations.map(
    (station) => station.name
    ),
  });
  ...
  while (
    ...
    const station = activeStations[index];
    const frequency = getStationFrequency(station);
...
function testStation(station) {
  ...
  // Mark station as healthy
  if (STATION_HEALTH_CONFIG.enabled) {
    setStationHealth((previous) => ({
      ...previous,
      [station.id]: "healthy",
    }));
  ...
  // Mark station as unhealthy
  if (STATION_HEALTH_CONFIG.enabled) {
    setStationHealth((previous) => ({
      ...previous,
      [station.id]: "unhealthy",
    }));
  }

  // Remember this station for the
  // current skip/recovery cycle.
...
async function skipFailedStation(station) {
  ...
  console.log(
    "FAILED STATION INDEX:",
    currentIndex
  );

  // Station isn't part of the currently
  // selected list.
  if (currentIndex === -1) {

    console.log(
      "FAILED STATION IS NOT IN ACTIVE LIST"
    );
   ...
   * IMPORTANT:
   *
   * Only search stations AFTER the failed
   * station inside the CURRENT active list.
   *
   * Do NOT use the global stations array.
   */
  ...
  for (
    ...
    // Do NOT wrap around here.
    // We only want to search the remaining
    // stations after the failed station.
    if (
      nextIndex >= activeStations.length
    ) {
      break;
    }
  ...
  /*
   * We reached the end of the CURRENT list.
   *
   * Example:
   *
   * Region has:
   ...
   *   C
   *
   * A failed → try B → try C
   *
   * If B and C also fail:
   * STOP.
   *
   * We do NOT jump into the global 100.
   */
...
{showPresets && (
      ...
      {availableRegions.map(
        (region) => (
          ...
          <span>
            ...
            {stationFilter === "favorites" ? favorites.length : stationFilter === "regions" ? displayedRegionStations.length
            : stations.length}
          </span>
  ...
  <strong>
    <span className={`station-health ${
      stationHealth[station.id] || "unknown"}`}
      title={
        stationHealth[station.id] === "healthy"
        ? "Station healthy"
        : stationHealth[station.id] === "unhealthy"
        ? "Station unavailable"
        : "Station not tested"
      }>
    ●
